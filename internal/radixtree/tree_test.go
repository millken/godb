...
func TestDeletePrefix(t *testing.T) {
	...
	rt.Put([]byte("torn"), "TORN")
	rt.Put([]byte("tag"), "TAG")
	rt.Put([]byte("tornado"), "TORNADO")
	prevSize := rt.Len()

	if rt.DeletePrefix([]byte("tox")) {
		t.Fatal("should not have deleted prefix")
	}

	if !rt.DeletePrefix([]byte("tom")) {
	...
	if rt.DeletePrefix([]byte("torx")) {
		t.Fatal("deleted prefix")
	}

	if !rt.DeletePrefix([]byte("tor")) {
		t.Fatal("did not delete prefix")
	}
...
func TestBuildEdgeCases(t *testing.T) {
	...
	tree.Put([]byte("ABCD"), 1)
	t.Log(dump(tree))
	tree.Put([]byte("ABCDE"), 2)
	t.Log(dump(tree))
	tree.Put([]byte("ABCDF"), 3)
	t.Log(dump(tree))

	val, ok := tree.Get([]byte("ABCE"))
	if ok || val != 0 {
		t.Fatal("expected no value")
	}

	if tree.Delete([]byte("ABCE")) {
		t.Fatal("should not delete non-existent value")
	}

	tree.Put([]byte("ABCE"), 4)
	t.Log(dump(tree))

	tree.Put([]byte("ABCDEFGHIJK"), 5)
	if tree.Delete([]byte("ABCDEFGH")) {
		t.Fatal("should not delete non-existent value")
	}
...
func TestSimpleIterAt(t *testing.T) {
	...
	count := 0
	for range rt.IterAt([]byte("tomato")) {
		count++
	}
	if count != 1 {
...
func TestTree(t *testing.T) {
	...
	for _, key := range keys {
		if val, ok := tree.Get([]byte(key)); ok {
	...
	// store keys
	for _, key := range keys {
		if isNew := tree.Put([]byte(key), "first"); !isNew {
	...
	for _, key := range keys {
		if isNew := tree.Put([]byte(key), strings.ToUpper(key)); isNew {
	...
	for _, key := range keys {
		...
		if val != strings.ToUpper(string(key)) {
	...
	t.Log(dump(tree))
	key := "bad/key"
	for key, val := range tree.IterPath([]byte(key)) {
	...
	kvMap = map[string]string{}
	wvals = nil
	for key, val := range tree.IterPath([]byte(lastKey)) {
		kvMap[string(key)] = val
		wvals = append(wvals, val)
	}
	if kvMap[lastKey] == "" {
	...
	t.Log(dump(tree))
	for _, key := range keys {
		if deleted := tree.Delete([]byte(key)); !deleted {
...
func TestNilGet(t *testing.T) {
	...
	one := 1
	tree.Put([]byte("/rat"), &one)
	two := 2
	tree.Put([]byte("/ratatattat"), &two)
	tree.Put([]byte("/ratatouille"), nil)

	val, ok := tree.Get([]byte("/ratatouille"))
	if !ok {
...
func TestRoot(t *testing.T) {
	...
	if !tree.Put([]byte(""), "hello") {
		t.Error("expected nil key to be new")
	}
	testVal := "world"
	if tree.Put([]byte(""), testVal) {
...
func TestIter(t *testing.T) {
	...
	for _, notKey := range notKeys {
		for key, val := range tree.IterAt([]byte(notKey)) {
			if val != strings.ToUpper(string(key)) {
				t.Fatalf("expected key %s to have value %v, got %v", string(key), strings.ToUpper(string(key)), val)
			}
			visited[string(key)]++
		}
	}
	...
	clear(visited)
	for key := range tree.IterAt([]byte("rat")) {
		visited[string(key)]++
	}
	if visited[keys[0]] != 0 {
	...
	clear(visited)
	for key := range tree.IterAt([]byte("rat/whis/kers")) {
		visited[string(key)]++
	}
	for _, key := range keys {
	...
	tree.Put([]byte(testKey), strings.ToUpper(testKey))

	for key, val := range tree.IterPath([]byte(testKey)) {
		v := strings.ToUpper(string(key))
		if val != v {
	...
	clear(visited)
	for key := range tree.IterPath([]byte(testKey)) {
		pfx := "rat/winks/wryly"
		if strings.HasPrefix(string(key), pfx) && len(key) > len(pfx) {
			continue
		}
		visited[string(key)]++
	}
	...
	var lastKey string
	for key := range tree.IterPath([]byte("rat/winks/wisely/x/y/z/w")) {
		lastKey = string(key)
	}
...
func TestIterStop(t *testing.T) {
	...
	for i := range table {
		tree.Put([]byte(table[i].key), table[i].val)
	}
...
func TestInspectStop(t *testing.T) {
	...
	var keys []string
	inspectFn := func(link, prefix, key []byte, depth, children int, hasValue bool, value int) bool {
		if !hasValue {
...
func TestGetAfterDelete(t *testing.T) {
	...
	// store keys
	for _, key := range keys {
		tree.Put([]byte(key), strings.ToUpper(key))
	}

	t.Log("Before")
	t.Log(dump(tree))

	if !tree.Delete([]byte("bird")) {
		t.Fatal("should have deleted bird")
	}
	t.Log("After")
	t.Log(dump(tree))

	_, ok := tree.Get([]byte("rat"))
	if !ok {
...
func dump[T any](tree *Tree[T]) string {
	...
	tree.Inspect(func(link, prefix, key []byte, depth, children int, hasValue bool, value T) bool {
		for i := 0; i < depth; i++ {
			b.WriteString("  ")
		}
		if hasValue {
			b.WriteString(fmt.Sprintf("%s-> (%q, [%s: %v]) children: %d\n", string(link), string(prefix), string(key), value, children))
		} else {
			b.WriteString(fmt.Sprintf("%s-> (%q, [%s]) children: %d\n", string(link), string(prefix), string(key), children))
		}
		return false
	})
	return b.String()
